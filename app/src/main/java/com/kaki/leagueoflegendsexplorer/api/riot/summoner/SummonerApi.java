package com.kaki.leagueoflegendsexplorer.api.riot.summoner;

import android.content.Context;
import android.util.Log;
import android.widget.Toast;

import com.kaki.leagueoflegendsexplorer.R;
import com.kaki.leagueoflegendsexplorer.api.HttpRequest;
import com.kaki.leagueoflegendsexplorer.api.riot.RequestInterception;
import com.kaki.leagueoflegendsexplorer.api.riot.summoner.models.SummonerDto;

import java.util.Map;

import retrofit.Callback;
import retrofit.RestAdapter;
import retrofit.RetrofitError;
import retrofit.client.Response;

/**
 * Created by kevinrodrigues on 12/07/15.
 */
public class SummonerApi {

    private SummonerRoutes api;

    public SummonerApi(Context context) {
        RestAdapter restAdapter = new RestAdapter.Builder()
                .setLogLevel(RestAdapter.LogLevel.FULL)
                .setEndpoint("https://euw.api.pvp.net")
                .setRequestInterceptor(new RequestInterception(context.getResources()))
                .build();

        api = restAdapter.create(SummonerRoutes.class);
    }

    public void getSummonerByName(final Context context, final String name, final HttpRequest<Map<String, SummonerDto>> request) {
        api.getSummonerByName("euw", name, new Callback<Map<String, SummonerDto>>() {
            @Override
            public void success(Map<String, SummonerDto> stringSummonerDtoMap, Response response) {
                Log.d("response", response.getBody().toString());
                request.success(stringSummonerDtoMap, response);
            }

            @Override
            public void failure(RetrofitError error) {
                if (error.getResponse() == null) {
                    Toast.makeText(context, R.string.no_internet, Toast.LENGTH_LONG).show();
                } else {
                    switch (error.getResponse().getStatus()) {
                        case 400:
                        case 401:
                        case 429:
                        case 500:
                        case 503:
                            Toast.makeText(context, R.string.error_with_server, Toast.LENGTH_LONG).show();
                            break;
                    }
                }
                request.failure(error);
            }
        });
    }
}
